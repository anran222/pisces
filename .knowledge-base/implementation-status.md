...
- 两条 MySQL 持久化链路都已统一改为 `repository + entity + mapper interface + mapper.xml`
- 但 assignment / exposure / 聚合事实当前仍主要存于 Redis
...
这些接口会抛出 `SERVICE_UNAVAILABLE` 类业务异常。

## 4. 当前设计上的重要约束

### 4.1 Redis 是事实上的运行时依赖
...
- 目前没有单独的快照审批/审核流程
- 快照生成依赖实时分析结果，因此 Redis 事实数据异常仍会传导到快照内容

## 5. 维护建议

- 若要长期可用，优先补齐“MySQL 持久化配置的生产化部署与迁移方案”
